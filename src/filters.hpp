...
namespace hf {
    ...
    class Filter {
        ...
        public:
            ...
            static float invSqrt(float x) // used ONLY in original madgwick code
            {
	            float halfx = 0.5f * x;
	            float y = x;
	            long i = *(long*)&y;
    ...
    class ExpMovAvg {
        public:
            ...
            {
                _sigma = (_alpha * aVal) + ((1-_alpha)*_sigma);
                return _sigma;
            }
    }; // class ExpMovAvg
        ...
        public:
            ...
            /*{
                ...
                if(!((ax == 0.0f) && (ay == 0.0f) && (az == 0.0f))) {
                    ...
                    recipNorm = Filter::invSqrt(ax * ax + ay * ay + az * az);
                    ax *= recipNorm;
                    ay *= recipNorm;
                    az *= recipNorm;   

                    // Normalise magnetometer measurement
                    recipNorm = Filter::invSqrt(mx * mx + my * my + mz * mz);
                    mx *= recipNorm;
                    my *= recipNorm;
                    mz *= recipNorm;
                    ...
                    recipNorm = Filter::invSqrt(s0 * s0 + s1 * s1 + s2 * s2 + s3 * s3); // normalise step magnitude
                    s0 *= recipNorm;
                    s1 *= recipNorm;
                    s2 *= recipNorm;
                    s3 *= recipNorm;
                ...
                recipNorm = Filter::invSqrt(q0 * q0 + q1 * q1 + q2 * q2 + q3 * q3);
                q0 *= recipNorm;
                q1 *= recipNorm;
                q2 *= recipNorm;
                q3 *= recipNorm;
